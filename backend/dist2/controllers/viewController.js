 ...
 */
import { logCandidateView, getUserViewHistory, getCandidateViewers, getUserViewStats, } from '../services/viewService.js';
import { validateViewLogRequest, validateEmail, validateCandidateId, validateViewHistoryFilters, } from '../utils/validators.js';
/**
 * Log a candidate view
 * POST /api/candidates/:id/view
 */
export async function logView(req, res, next) {
    try {
 ...
 */
export async function getUserViewHistoryHandler(req, res, next) {
    try {
 ...
 */
export async function getCandidateViewersHandler(req, res, next) {
    try {
        const candidateId = validateCandidateId(req.params.id);
        const filters = validateViewHistoryFilters(req.query);
 ...
 */
export async function getUserViewStatsHandler(req, res, next) {
    try {
        const email = validateEmail(req.params.email);
