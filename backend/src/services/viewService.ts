...
export async function logCandidateView(
  candidateId: string,
  userData: ViewLogRequest
): Promise<ViewLogResponse> {
  try {
 ...
 */
export async function getCandidateViewers(
  candidateId: string,
  filters: ViewHistoryFilters
): Promise<CandidateViewers> {
  try {
    ...
    const uniqueViewers = new Set((uniqueViewersData || []).map((v: any) => v.user_id)).size
 ...
 */
export async function getUserViewStats(email: string): Promise<UserViewStats> {
  try {
    // 1. Find user
    const { data: user, error: userError } = await supabase
      .from('users')
    ...
    const uniqueCandidates = new Set(viewsList.map((v: any) => v.candidate_id)).size
