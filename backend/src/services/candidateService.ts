    ...
    if (filters.sort === 'assessment_avg') {
      candidates.sort((a, b) => {
        ...
        if (a.assessmentScore !== 'N/A') {
          const [studentScore, maxScore] = a.assessmentScore.split(' / ').map((s: string) => parseFloat(s) || 0)
          aScore = maxScore > 0 ? (studentScore / maxScore) * 100 : 0
        }
        
        if (b.assessmentScore !== 'N/A') {
          const [studentScore, maxScore] = b.assessmentScore.split(' / ').map((s: string) => parseFloat(s) || 0)
